...
G_DEFINE_TYPE(Expression, expression, CLASSMODEL_TYPE)
...
expression_get_itext_sub(Row *row)
{
	 ...
	 */
	if (row->sym &&
		g_str_equal(IOBJECT(row->sym)->name, MEMBER_EXPR) &&
		row->child_rhs &&
...
expression_get_itext(Expression *expression)
{
	...
	if (row->child_rhs &&
		row->child_rhs->scol)
		return ITEXT(subcolumn_map(SUBCOLUMN(row->child_rhs->scol),
			(row_map_fn) expression_get_itext_sub, NULL, NULL));
...
expression_class_get(Classmodel *classmodel, PElement *root)
{
	...
	if (!class_get_member_string(root, MEMBER_CAPTION, caption, MAX_STRSIZE))
		return FALSE;
