...
program_parse(Program *program)
{
    ...
    if (strspn(txt, WHITESPACE ";") == strlen(txt))
        return TRUE;
    ...
    if (!parse_toplevel(program->kit, program->tool_pos)) {
        text_view_select_text(GTK_TEXT_VIEW(program->text_view),
            get_input_state()->charpos - yyleng, get_input_state()->charpos);
    ...
    /* Reselect last_sym, the last thing the parser saw. This might be a
	 * secondary def, make sure we have the top one.
     */
    if (compile->last_sym) {
		Symbol *top = symbol_get_top(compile->last_sym);

		if (top->tool)
			program_set_tool(program, top->tool);
	}
