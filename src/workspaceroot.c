...
G_DEFINE_TYPE(Workspaceroot, workspaceroot, MODEL_TYPE)
...
{
	ICONTAINER_CLASS(workspaceroot_parent_class)->child_add(parent, child, pos);
...
{
	ICONTAINER_CLASS(workspaceroot_parent_class)->child_remove(parent, child);
}
...
workspaceroot_new(const char *name)
{
	...
	wsr = WORKSPACEROOT(g_object_new(WORKSPACEROOT_TYPE, NULL));
