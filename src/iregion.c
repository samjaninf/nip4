...
{
	ICONTAINER_CLASS(iregion_parent_class)->parent_add(child);
...
{
	/* Get our parent class. We can't just use the global iregion_parent_class,
	...
	ModelLoadState *state, Model *parent, xmlNode *xnode)
{
	...
	return MODEL_CLASS(parent_class)->load(model, state, parent, xnode);
}
...
iregion_class_new(Classmodel *classmodel, PElement *fn, PElement *out)
{
	...
	if (instance) {
		...
		if (!heap_appl_add(heap, out, &rhs) ||
			!heap_element_new(heap, &instance->image_class, &rhs) ||
