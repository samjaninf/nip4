...
G_DEFINE_TYPE(Rhs, rhs, HEAPMODEL_TYPE)
	...
	ModelLoadState *state, Model *parent, xmlNode *xnode)
{
	...
	if (!MODEL_CLASS(rhs_parent_class)->load(model, state, parent, xnode))
		return FALSE;
...
rhs_refresh_graphic(Rhs *rhs, PElement *root)
{
	...
	if (result &&
		...
		for (i = 0; i < VIPS_NUMBER(rhs_graphic); i++) {
