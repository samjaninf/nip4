...
G_DEFINE_TYPE(Expr, expr, ICONTAINER_TYPE)
...
expr_map_all(Expr *expr, map_expr_fn fn, void *a)
{
	...
	/* And over any locals.
	 */
	if (expr->compile && (res = (Expr *) icontainer_map(ICONTAINER(expr->compile), (icontainer_map_fn) expr_map_all_sub, (void *) fn, a)))
		return res;
...
expr_strip(Expr *expr)
{
	...
	/* Junk error stuff.
	 */
	VIPS_FREE(expr->error_top);
	VIPS_FREE(expr->error_sub);
