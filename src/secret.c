...
secret_find_nonlocal(Compile *compile)
{
	 ...
	 */
	if (slist_map(compile->children, (SListMapFn) secret_is_nonlocal, compile))
		return compile;
...
{
	return compile_map_all(compile, (map_compile_fn) secret_all_add, param);
}
...
secret_build(Compile *compile)
{
	 ...
	 */
	(void) compile_map_all(compile, (map_compile_fn) secret_set_class, NULL);
