...
G_DEFINE_TYPE(Pathname, pathname, CLASSMODEL_TYPE)
...
pathname_class_init(PathnameClass *class)
{
	...
	classmodel_class->n_members = VIPS_NUMBER(pathname_members);
...
pathname_init(Pathname *pathname)
{
	 ...
	 */
	pathname->value = NULL;
	VIPS_SETSTR(pathname->value, "no-file");
