...
{
	return SYMBOL(icontainer_child_lookup(ICONTAINER(compile), name));
}
...
compile_expr_link_make(Compile *compile, Expr *expr)
{
	...
	iobject_ref_sink(IOBJECT(compile));
}
...
compile_new_toplevel(Expr *expr)
{
	...
	iobject_ref_sink(IOBJECT(compile->heap));
	...
	if (hn->type == TAG_REFERENCE) {
		...
		if (hn1->type != TAG_SHARED) {
			...
			*hn2 = *hn1;
			...
			PPUT(hn1,
				ELEMENT_NODE, hn2,
				ELEMENT_CHAR, GUINT_TO_POINTER(compile_share_number));
