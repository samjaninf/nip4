...
itext_finalize(GObject *gobject)
{
	 ...
	 */
	VIPS_FREE(itext->formula);
	VIPS_FREE(itext->formula_default);
...
itext_update_model(Heapmodel *heapmodel)
{
	...
	return HEAPMODEL_CLASS(itext_parent_class)->update_model(heapmodel);
}
...
itext_update_heap(Heapmodel *heapmodel)
{
	...
	return HEAPMODEL_CLASS(itext_parent_class)->update_heap(heapmodel);
}
...
itext_clear_edited(Heapmodel *heapmodel)
{
	...
	return HEAPMODEL_CLASS(itext_parent_class)->clear_edited(heapmodel);
}
	...
	ModelLoadState *state, Model *parent, xmlNode *xnode)
{
	...
	return MODEL_CLASS(itext_parent_class)->load(model, state, parent, xnode);
}
...
itext_set_formula(iText *itext, const char *formula)
{
	if (!itext->formula || strcmp(itext->formula, formula) != 0) {
		...
		VIPS_SETSTR(itext->formula, formula);
