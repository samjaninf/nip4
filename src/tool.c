...
toolitem_free(Toolitem *toolitem)
{
	...
	VIPS_FREE(toolitem->label);
	VIPS_FREE(toolitem->name);
	VIPS_FREE(toolitem->icon);
	VIPS_FREE(toolitem->tooltip);
	VIPS_FREE(toolitem->help);
	VIPS_FREE(toolitem->action);
	VIPS_FREE(toolitem->path);
	VIPS_FREE(toolitem->user_path);
	VIPS_FREE(toolitem);
...
toolitem_set_name(Toolitem *toolitem, PElement *root)
{
	...
	if (root &&
		...
		if (class_get_member_string(root,
			 ...
			 */
			VIPS_SETSTR(toolitem->label, _(value));
			...
			for (p = q = value; *p; p++)
				if (*p != '_')
					*q++ = *p;
			*q = '\0';
			VIPS_SETSTR(toolitem->name, value);
		}

		if (class_get_member_string(root,
				MEMBER_ICON, value, MAX_NAME))
			VIPS_SETSTR(toolitem->icon, value);
	}
	else {
		...
		VIPS_SETSTR(toolitem->label, value);
		VIPS_SETSTR(toolitem->name, toolitem->label);
	}
...
toolitem_set_tooltip(Toolitem *toolitem, PElement *root)
{
	...
	if (root &&
		...
		toolitem->tool->help)
		VIPS_SETSTR(toolitem->tooltip, toolitem->tool->help);
}
...
toolitem_set_action(Toolitem *toolitem, PElement *root)
{
	...
	VIPS_SETSTR(toolitem->action, vips_buf_all(&buf));
...
toolitem_set_path(Toolitem *toolitem)
{
	...
	VIPS_SETSTR(toolitem->path, vips_buf_all(&buf));
}
...
toolitem_set_help(Toolitem *toolitem)
{
	...
	VIPS_SETSTR(toolitem->help, vips_buf_firstline(&buf));
}
...
toolitem_lookup_toolkit(Toolkit *kit, Symbol *action)
{
	return (Toolitem *) toolkit_map(kit,
		(tool_map_fn) toolitem_lookup_tool, action, NULL);
}
...
toolitem_lookup(Toolkitgroup *kitg, Symbol *action)
{
	return (Toolitem *) toolkitgroup_map(kitg,
		(toolkit_map_fn) toolitem_lookup_toolkit, action, NULL);
}
