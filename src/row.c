...
row_dirty_set(Row *row, gboolean clear_error)
{
	...
	return icontainer_map_all(ICONTAINER(row),
		(icontainer_map_fn) row_dirty_set_sub, GINT_TO_POINTER(clear_error));
}
	...
	ModelLoadState *state, Model *parent, xmlNode *xnode)
{
	...
	if (!get_sprop(xnode, "name", name, 256))
		return FALSE;
	VIPS_SETSTR(IOBJECT(row)->name, name);
	...
	if (!MODEL_CLASS(row_parent_class)->load(model, state, parent, xnode))
		return FALSE;
...
row_new_heap(Heapmodel *heapmodel, PElement *root)
{
	...
	return HEAPMODEL_CLASS(row_parent_class)->new_heap(heapmodel, root);
}
...
row_update_model(Heapmodel *heapmodel)
{
	...
	return HEAPMODEL_CLASS(row_parent_class)->update_model(heapmodel);
}
