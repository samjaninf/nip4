...
G_DEFINE_TYPE(iContainer, icontainer, IOBJECT_TYPE)
...
{
	return slist_map4(icontainer->children, (SListMap4Fn) fn, a, b, c, d);
}
...
{
	return slist_map5(icontainer->children, (SListMap5Fn) fn, a, b, c, d, e);
}
...
{
	return slist_map2_rev(icontainer->children, (SListMap2Fn) fn, a, b);
}
...
{
	g_autofree char *one = g_utf8_casefold(IOBJECT(a)->name, -1);
	g_autofree char *two = g_utf8_casefold(IOBJECT(b)->name, -1);
...
icontainer_child_move(iContainer *child, int pos)
{
	...
	if (pos >= 0)
		parent->children = g_slist_insert(parent->children, child, pos);
	else
...
icontainer_dispose(GObject *gobject)
{
	...
	printf("icontainer_dispose: (%p) %s \"%s\"\n",
		icontainer, G_OBJECT_TYPE_NAME(icontainer), IOBJECT(icontainer)->name);
#endif /*DEBUG*/
...
icontainer_finalize(GObject *gobject)
{
	...
	VIPS_FREEF(g_hash_table_destroy, icontainer->child_hash);
...
icontainer_link(iContainer *parent, iContainer *child, int pos)
{
	...
	if (parent->child_hash) {
		...
		g_hash_table_insert(parent->child_hash, IOBJECT(child)->name, child);
	}
}
...
icontainer_unlink(iContainer *child)
{
	...
	child->parent = NULL;
	if (parent->child_hash) {
		g_assert(g_hash_table_lookup(parent->child_hash, IOBJECT(child)->name));

		g_hash_table_remove(parent->child_hash, IOBJECT(child)->name);
	}
}
